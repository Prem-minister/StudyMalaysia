...
if(isset($_POST["signup-submit"])){
    ...
    elseif(!preg_match("/^[a-zA-Z ]+$/", $name)){
        header("Location: student_register.php?error=invalidName");
        exit();
    }
    elseif(!preg_match("/^[0-9]+-[0-9]+$/", $phone)){
        header("Location: student_register.php?error=InvalidContactNumber");
        exit();
    }
    else if(!preg_match("/^[A-Za-z][A-Za-z0-9]*[?:_A-Za-z0-9]*$/",$username)){
        header("Location: student_register.php?error=invalidUsername&username=".$username);
        exit();
    }
    else if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
        header("Location: student_register.php?error=email");
        exit();
    }

    else if($password !== $password_r){
        header("Location: student_register.php?error=passwordCheck&username=usernam&email=email");
        exit();
    }   
    else{
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)){
            ...
            if($resultCount > 0){
                header("Location: student_register.php?error=userTaken&mail=email");
                exit();
            }else{
                $sql = "INSERT INTO student (`Name`,`Gender`,`Email`,`Contact_Number`,`Address`,`Country`,`Date_of_Birth`,`Username`,`Password`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
                $stmt = mysqli_stmt_init($conn);
                if(!mysqli_stmt_prepare($stmt,$sql)){
                    header("Location: student_register.php?error=sqlError");
                    exit();
                    ...
                    mysqli_stmt_execute($stmt);
                    header("Location: student_register.php?submit=success");
                    exit();
                }
